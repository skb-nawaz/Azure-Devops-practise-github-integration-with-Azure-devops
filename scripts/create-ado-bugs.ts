...
`,
  ...
  for (const attachment of bug.attachments || []) {
    try {
      ...
      await attachFileToBug(bugId, attachmentUrl, attachment.name);

      console.log(`✅ Attached ${attachment.name}`);
    } catch (error: any) {
      console.log(`❌ Failed to upload ${attachment.name}`);
